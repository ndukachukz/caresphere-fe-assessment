...
import { CARESPHERE_FLUID_COLORS } from "#/shared/theme/colors";
...
const prepColors = (input?: string[]) => {
	const base = (input?.length ? input : [...CARESPHERE_FLUID_COLORS]).slice(
		0,
		MAX_COLORS,
	);
...
export const Ferrofluid: React.FC<FerrofluidProps> = ({
	...
	dpr,
	paused = false,
	colors = CARESPHERE_FLUID_COLORS,
	speed = 0.5,
	scale = 1.6,
	turbulence = 1,
	fluidity = 0.1,
	...
	const settingsRef = useRef({ paused, mouseDampening });
	...
	useEffect(() => {
		...
		const { dpr: dprProp, colors: colorsProp } = propsRef.current;
		...
		const { arr, count, avg } = prepColors(colorsProp);
		...
		const uniforms: Uniforms = {
			...
			uFlow: { value: flowVec(propsRef.current.flowDirection) },
			uSpeed: { value: propsRef.current.speed },
			uScale: { value: propsRef.current.scale },
			uTurbulence: { value: propsRef.current.turbulence },
			uFluidity: { value: propsRef.current.fluidity },
			uRimWidth: { value: propsRef.current.rimWidth },
			uSharpness: { value: propsRef.current.sharpness },
			uShimmer: { value: propsRef.current.shimmer },
			uGlow: { value: propsRef.current.glow },
			uOpacity: { value: propsRef.current.opacity },
			uMouseEnabled: {
				value: propsRef.current.mouseInteraction ? 1 : 0,
			},
		...
		const loop = (t: number) => {
			...
			if (settings.mouseDampening > 0) {
				...
				const tau = Math.max(1e-4, settings.mouseDampening);
			...
			if (!settings.paused && meshRef.current) {
				try {
	...
	useEffect(() => {
		...
		settingsRef.current = { paused, mouseDampening };
